...
// NewPluginDiscovery creates a new PluginDiscovery instance. What it finds is
// reported as discover log events to emit, a nil emit is silent.
func NewPluginDiscovery(directories []string, pattern string, emit events.Emit) *PluginDiscovery {
	if pattern == "" {
		pattern = "xcl-plugin-*"
	}
	...
	return &PluginDiscovery{
		...
		pattern:     pattern,
		logger:      logger.New(emit, events.Event{Source: events.SourceCore, Operation: events.OperationDiscover}),
	}
}
